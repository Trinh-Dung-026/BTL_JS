...
const sliderTrack = document.querySelector('.slider-track'); // Giữ nguyên
...
function updateSlider() {
    ...
    if (minVal > maxVal) {
        ...
        minVal = maxVal;
        maxVal = temp;
        // Cập nhật lại giá trị của input nếu cần hoán đổi
    ...
    // (Giá trị hiện tại - min) / (max - min) * chiều rộng slider
    ...
    if (currentMinRight + gapThreshold > calculatedMaxLeft) {
        ...
        // Đơn giản nhất là đẩy minPriceDisplay lùi lại một chút
    ...
    minPriceDisplay.style.top = `12px`; // Điều chỉnh để nằm trên thanh trượt
    maxPriceDisplay.style.top = `12px`; // Điều chỉnh để nằm trên thanh trượt
}
    ...
    // Cập nhật lại vị trí khi kích thước cửa sổ thay đổi (để sliderWidth đúng)
